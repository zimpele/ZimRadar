...
PIPELINE_STEPS = [
    {
        ...
        "color": "#1565C0",
        "what": "Every county in California & Florida is photographed from space every 5 days by the European Space Agency's Sentinel-2 satellites.",
        "how": [
            "Downloads cloud-free imagery for each county",
            "Stores tiles in a time-series database",
            "Re-fetches automatically as new images arrive",
        ],
        "tech": "Sentinel-2 STAC API · Prefect pipeline",
    },
    {
        "emoji": "🔬",
        "title": "AI Image Analysis",
        "color": "#2E7D32",
        "what": "A deep learning model reads each satellite image and measures exactly how much of the county is forest, farmland, or city — and how that changed over time.",
        "how": [
            "Detects vegetation loss (wildfires, deforestation)",
            "Measures urban density from above",
            "Estimates terrain depth without ground sensors",
        ],
        "tech": "SegFormer (HuggingFace) · ZoeDepth monocular depth",
    },
    {
        "emoji": "🌧️",
        "title": "Climate & Disaster Records",
        "color": "#00695C",
        "what": "We pull decades of weather records and every government-declared disaster to understand each county's climate history.",
        "how": [
            "20 years of precipitation trends from 1,000+ NOAA stations",
            "Every FEMA flood & hurricane declaration since 2000",
            "Elevation variance from USGS terrain data",
        ],
        "tech": "NOAA API · FEMA Disaster Declarations · USGS EPQS",
    },
    {
        "emoji": "🏗️",
        "title": "Infrastructure Age",
        "color": "#4527A0",
        "what": "Using crowdsourced map data, we estimate how old the buildings in each county are — older infrastructure is more vulnerable to climate damage.",
        "how": [
            "Counts every mapped building per county",
            "Extracts construction year tags where available",
            "Uses building density as a development-age proxy",
        ],
    ...
    },
    {
        ...
        "color": "#E65100",
        "what": "A machine learning model trained on historical disaster patterns combines all 12 risk factors and assigns each county a climate risk tier.",
        "how": [
            "12 features: flood history, vegetation loss, urban density, elevation, infrastructure age + more",
            "Trained on real FEMA disaster data with class-balanced weights",
            "SHAP values explain *why* each county got its score",
        ],
        "tech": "XGBoost · SHAP explainability · AUC 0.91",
    },
    {
        "emoji": "🗺️",
        "title": "Interactive Risk Map",
        "color": "#880E4F",
        "what": "Every county appears on a live map with its risk level, a bar chart showing which factors drove the score, and a plain-English narrative generated by a local AI.",
        "how": [
            "Color-coded map: green → red by risk tier",
            "SHAP waterfall chart per county",
            "AI narrative generated locally by Gemma 2 (no data leaves the server)",
        ],
        "tech": "Streamlit · Folium · Plotly · Gemma 2 via Ollama",
    },
]

with tab_arch:
    st.markdown(
        "### How ZimRadar works — from satellite to risk score\n"
        "ZimRadar automatically collects data from six sources, analyzes it with AI, "
        "and produces a county-level climate risk assessment — fully automated, end-to-end."
    )
    st.divider()

    # Pipeline story cards
    for i, step in enumerate(PIPELINE_STEPS):
        col_icon, col_content = st.columns([1, 7])
        with col_icon:
            st.markdown(
                f"<div style='font-size:3rem; text-align:center; padding-top:8px'>{step['emoji']}</div>",
                unsafe_allow_html=True,
            )
            if i < len(PIPELINE_STEPS) - 1:
                st.markdown(
                    "<div style='text-align:center; font-size:1.4rem; color:#aaa'>↓</div>",
                    unsafe_allow_html=True,
                )
        with col_content:
            st.markdown(
                f"<span style='background:{step['color']};color:white;padding:2px 10px;"
                f"border-radius:4px;font-weight:600;font-size:0.85rem'>Step {i + 1}</span>"
                f" &nbsp; **{step['title']}**",
                unsafe_allow_html=True,
            )
            st.markdown(step["what"])
            for bullet in step["how"]:
                st.markdown(f"- {bullet}")
            st.caption(f"Technology: {step['tech']}")
        if i < len(PIPELINE_STEPS) - 1:
            st.write("")

    st.divider()

    # Technical graph behind expander
    with st.expander("🛠️ Technical: module dependency graph (for developers)"):
        from src.dashboard.codebase_graph import build_graph_data, build_plotly_figure

        use_llm = st.toggle(
            "🤖 Enhance undocumented functions with Gemma2",
            value=False,
            help="Calls the local Gemma2:9b model to generate plain-English descriptions. "
            "First run ~2 min; results are cached.",
        )
        ...
        c1.metric("Modules", len(graph_data["nodes"]))
        c2.metric("Functions / Classes", sum(len(n["functions"]) for n in graph_data["nodes"]))
        c3.metric("Import edges", len(graph_data["edges"]))

        st.plotly_chart(build_plotly_figure(graph_data), use_container_width=True)

        query = st.text_input("🔍 Search functions", placeholder="e.g. classify, fema, sentinel")
        rows = []
